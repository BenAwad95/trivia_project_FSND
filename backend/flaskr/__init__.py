...
def create_app(test_config=None):
  ...
  def get_questions():
    ...
    # check for if there questions in the page
  ...
  def delete_question(question_id):
    ...
    question.delete()
    flash(f"The question '{question_text}' was deleted.")
    return jsonify({
      'success': True,
      'massage': f"The question '{question_text}' was deleted."
    })
  '''
  @TODO: 
  ...
  def questuions_search():
    try:
      search_term = request.get_json()['searchTerm']
      questions = [question.format() for question in Question.query.filter(Question.question.ilike(f'%{search_term}%'))]
      return jsonify({
        'questions': questions,
  ...
  def get_category_questiona(category_id):
    category = Category.query.get(category_id)
    if category is None:
      abort(404)
    questions = Question.query.filter(Question.category.ilike(category.type))
  ...
  def get_next_question():
    ...
    pre_que_ids = data['previous_questions']
    ...
    if category['id'] == 0:
      ...
      questions = [question.format() for question in Question.query.filter(Question.category.ilike(category['type']))]
      question = {}
      for q in questions:
        if q['id'] not in pre_que_ids:
          question = q
